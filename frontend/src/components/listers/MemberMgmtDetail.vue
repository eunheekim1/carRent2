<template>
    <v-card outlined>
        <v-card-title>
            MemberMgmt # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Number label="MileagePoint" v-model="item.mileagePoint" :editMode="editMode" @change="change" />
            </div>
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/memberMgmts/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
